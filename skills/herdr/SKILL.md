...
Create a sibling pane with the same `herdr pane split --current --direction <right|down> --cwd "$PWD" --no-focus` command shown above, which preserves the caller's working directory and focus.
...
After that failed read, ask the agent to write its complete response as Markdown in the run's temporary directory (`<state_dir>/tmp/`, avoiding system `/tmp`) and reply only with the file path, then read the file directly. Use this only as a fallback; do not request file output in the initial prompt.
...
## Observed 2026-09-20: dispatch traps
- `herdr agent prompt <agent> "$(cat long-brief.md)"` returned `{"err":null}` with the agent still `idle` and an empty composer: a multi-paragraph brief pasted as the prompt argument did not land at all. Send a one-line pointer instead ("Read and carry out the brief at <abs path> exactly ...") and keep the brief on disk in the run's `tmp/`. Confirm with `agent get` reporting `working` (codex) or pane output (agy) before counting the dispatch as in flight. Note `herdr agent prompt --timeout` is rejected without `--wait`.
...
## Compact-then-queue reuse
Reusing an executor or plan_reviewer over the 272,000-token threshold: send `agent prompt <target> /compact`, then immediately queue the next brief pointer to that same target — back to back, no `--wait`, no poll for compaction to finish. Still confirm each prompt landed (per the dispatch trap above, `{"err":null}` can leave the composer empty): the pane shows `/compact` running and the brief queued. That is a delivery check, not a wait for compaction. `scripts/office_runtime.py reuse-plan` computes this decision (`compact_then_queue` vs `normal_reuse`); its `commands` list is the dispatch order to run.
